...
public class Main {
    public static void main(String[] args) {
        ...
        */
        ...
        SourceCode[] codeArray = new SourceCode[fileNames.length];
        for (int i = 0; i < fileNames.length; i++) {
            codeArray[i] = new SourceCode(fileNames[i]);
        }

        List<SourceCode> codeList = new ArrayList<>();
        for (int i = 0; i < fileNames.length; i++) {
            codeList.add(new SourceCode(fileNames[i]));
        }

        Builder ide = new IDE(preprocessor, compiler, linker);
        ide.createProject("ArrayProject.exe");
        Executable exe1 = ide.build(codeArray);
        exe1.execute();

        ide.createProject("ListProject.exe");
        ide.build(codeArray);
        Executable exe2 = ide.build(codeArray);
        exe2.execute();
    }
}
