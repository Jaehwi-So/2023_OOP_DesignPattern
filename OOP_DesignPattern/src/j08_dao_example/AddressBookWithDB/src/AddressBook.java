package j08_dao_example.AddressBookWithDB.src;


public class AddressBook {
    public static void main(String[] args) {
        j08_dao_example.AddressBookWithDB.src.Person p;
        PersonDao personDao = new PersonDaoImpl();

        System.out.println("--- inserting...");
        p = new j08_dao_example.AddressBookWithDB.src.Person("Seonyoung Kim", "1 Hongji Dong");
        personDao.insert(p);
        p = new j08_dao_example.AddressBookWithDB.src.Person("Jangkwon Lee", "2 Hongji Dong");
        personDao.insert(p);

        System.out.println("--- finding all...");
        for (j08_dao_example.AddressBookWithDB.src.Person pi : personDao.findAll()) {
            System.out.println("reading... " + pi);
        }

        System.out.println("--- updating...");
        p = personDao.findAll().get(0);
        p.setName("Sooyoung Lim");
        personDao.update(p.getId(), p);

        System.out.println("--- see if updated...");
        p = personDao.findById(1);
        if (p != null) {
            System.out.println(p);
        }

        System.out.println("--- deleting...");
        personDao.delete(1);  // or personDao.delete(p);

        System.out.println("--- finding all after deleting...");
        for (Person pi : personDao.findAll()) {
            System.out.println("reading... " + pi);
        }
    }
}
